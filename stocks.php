...
<div class="card">
    ...
    <div class="card-body">
        <table class="table table-hover table-striped table-bordered">
            ...
            <tbody>
                <?php 
                $sql = "SELECT s.*,ss.name as sname,p.name as pname,p.product_code FROM `stock_list` s inner join `product_list` p on s.product_id = p.product_id inner join `supplier_list` ss on s.supplier_id = ss.supplier_id order by strftime('%s',date_added) desc";
                $qry = $conn->query($sql);
                $i = 1;
